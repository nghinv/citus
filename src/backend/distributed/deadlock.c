...
{
	int waitingPid;
	int waitinginitiatorNodeIdentifier;
	int64 waitingTransactionId;
	TimestampTz waitingTransactionStamp;

	int blockingPid;
	int blockinginitiatorNodeIdentifier;
...
static WaitGraph * BuildLocalWaitForGraph(int sourceinitiatorNodeIdentifier, int localinitiatorNodeIdentifier);
...
this_machine_kills_deadlocks(PG_FUNCTION_ARGS)
{
	...
	int localinitiatorNodeIdentifier = 0; /* FIXME: get correct local node id */
	int curBackend = 0;
	size_t curEdgeNum;
	DistributedTransactionShmemData *distributedTransactionShmemData= GetShmem();
	...
	LWLockAcquire(&distributedTransactionShmemData->lock, LW_SHARED);
	for (curBackend = 0; curBackend < MaxBackends; curBackend++)
	{
		DistributedTransactionBackendData *CurBackendData = &distributedTransactionShmemData->sessions[curBackend];
		PGPROC *curProc;
		...
		if (localinitiatorNodeIdentifier != CurBackendData->transactionId.initiatorNodeIdentifier)
		{
			continue;
		}
	...
	for (curEdgeNum = 0; curEdgeNum < waitGraph->numUsed; curEdgeNum++)
	{
		...
		DistributedTransactionId from;
		DistributedTransactionId to;
		LockDepNode *fromNode;
		LockDepNode *toNode;

		from.initiatorNodeIdentifier = curEdge->waitinginitiatorNodeIdentifier;
		from.transactionId = curEdge->waitingTransactionId;
		from.timestamp = curEdge->waitingTransactionStamp;

		to.initiatorNodeIdentifier = curEdge->blockinginitiatorNodeIdentifier;
...
{
	int32 sourceinitiatorNodeIdentifier = PG_GETARG_INT32(0);
	int32 localinitiatorNodeIdentifier = PG_GETARG_INT32(1);
	WaitGraph *waitGraph = NULL;

	waitGraph = BuildLocalWaitForGraph(sourceinitiatorNodeIdentifier, localinitiatorNodeIdentifier);
...
RecordWaitEdge(WaitGraph *waitGraph,
			   int localinitiatorNodeIdentifier, bool blockingAlsoBlocked,
			   DistributedTransactionBackendData *fromData, PGPROC *fromProc,
			   DistributedTransactionBackendData *toData, PGPROC *toProc)
{
	...
	{
		curEdge->waitinginitiatorNodeIdentifier = localinitiatorNodeIdentifier;
		curEdge->waitingTransactionId = 0;
		curEdge->waitingTransactionStamp = 0;
	}
	else
	{
		curEdge->waitinginitiatorNodeIdentifier = fromData->transactionId.initiatorNodeIdentifier;
	...
	{
		curEdge->blockinginitiatorNodeIdentifier = localinitiatorNodeIdentifier;
		curEdge->blockingTransactionId = 0;
		curEdge->blockingTransactionStamp = 0;
	}
	else
	{
		curEdge->blockinginitiatorNodeIdentifier = toData->transactionId.initiatorNodeIdentifier;
...
static WaitGraph *
BuildLocalWaitForGraph(int sourceinitiatorNodeIdentifier, int localinitiatorNodeIdentifier)
{
	...
	for (curBackend = 0; curBackend < MaxBackends; curBackend++)
	{
		...
		/* Skip if not not originating from interesting node */
		if (sourceinitiatorNodeIdentifier != CurBackendData->transactionId.initiatorNodeIdentifier)
		{
			continue;
		}
	...
	while (todoList)
	{
		...
		DistributedTransactionBackendData *curBackendData =
			&distributedTransactionShmemData->sessions[curProc->pgprocno];
		...
		if (curProc->links.next != NULL && curProc->waitLock != NULL)
		{
			...
			{
				PGPROC *leader;
				DistributedTransactionBackendData *blockingBackendData = NULL;
				PGPROC *nextProc;
				...
				blockingBackendData = &distributedTransactionShmemData->sessions[leader->pgprocno];
						 ...
						 lockMethodOff++)
					{
							...
							(conflictMask & LOCKBIT_ON(lockMethodOff)))
						{
							...
							RecordWaitEdge(waitGraph,
										   localinitiatorNodeIdentifier, blockingAlsoBlocked,
			 ...
			 */
			{
				...
				while (queueSize-- > 0)
				{
					...
					if ((LOCKBIT_ON(leader->waitLockMode) & conflictMask) != 0)
					{
						...
						blockingBackendData =
							&distributedTransactionShmemData->sessions[leader->pgprocno];
...
ReturnWaitGraph(WaitGraph *waitGraph, FunctionCallInfo fcinfo)
{
	...
	for (curEdgeNum = 0; curEdgeNum < waitGraph->numUsed; curEdgeNum++)
	{
		...
		values[1] = Int32GetDatum(curEdge->waitinginitiatorNodeIdentifier);
		...
		values[5] = Int32GetDatum(curEdge->blockinginitiatorNodeIdentifier);
...
WaitEdgeFromResult(WaitEdge *edge, PGresult *result, size_t rowIndex)
{
	...
	edge->waitinginitiatorNodeIdentifier = ParseIntField(result, rowIndex, 1, NULL);
	...
	edge->blockinginitiatorNodeIdentifier = ParseIntField(result, rowIndex, 5, &isNull);
...
TransactionIdHash(const void *key, Size keysize)
{
	...
	hash = hash_any((unsigned char *) &entry->initiatorNodeIdentifier, sizeof(int64));
...
{
	DistributedTransactionId *ta = (DistributedTransactionId *) a;
	DistributedTransactionId *tb = (DistributedTransactionId *) b;

	/* NB: Not used for sorting, just equality... */
	if (ta->initiatorNodeIdentifier != tb->initiatorNodeIdentifier ||
...
static LockDepNode *
LookupDepNode(HTAB *lockDepNodeHash, DistributedTransactionId *transactionId)
{
	bool found = false;
